...
_EOT
    ...
    echo "$IP_ADDRESS\twww.$VIRTUALHOST.dev" >> /etc/hosts
    echo "done"
fi
...
#     fi
    ...
    <VirtualHost *:$APACHE_PORT>
      ServerName www.$VIRTUALHOST.dev
      ServerAlias www.$VIRTUALHOST.dev
      DocumentRoot $DOC_ROOT_PREFIX/$FOLDER
      ErrorLog $ERROR_LOG/$VIRTUALHOST-error.log
      CustomLog $ERROR_LOG/$VIRTUALHOST-access.log combined
...
__EOF
    ...
    /usr/sbin/a2ensite $VIRTUALHOST.conf 1>/dev/null 2>/dev/null
    ...
    http://www.$VIRTUALHOST.dev/ is setup and ready for use.
...
__EOF
    ...
    sudo -u $USER -H xdg-open http://www.$VIRTUALHOST.dev/ &
    echo "done"
